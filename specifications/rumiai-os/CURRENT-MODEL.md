...
Version 2 also implements shared-local artifact restoration for incremental operations. After successful incremental execution, supported declared output files/directory trees may be published as verified non-authoritative user-scoped `mk` cache artifacts separately from project-scoped freshness metadata. Artifact bytes are keyed by the existing effective operation fingerprint and may therefore be reused across different canonical project roots without introducing a second operation identity. Restoration occurs only on the execution path; `--plan` remains non-mutating. A receiving checkout verifies/stages the selected immutable candidate, materializes outputs and writes its own canonical-project-root/operation freshness record. Invalid artifact cache state is a conservative miss. Successful restoration produces ordinary `up-to-date` evidence without fabricating process-result fields, so result observation still forces actual execution. Output-input consumers and provider-derived incremental members inherit the same ordinary operation restoration semantics. Concurrent publication uses fully verified immutable candidates plus an atomic per-fingerprint selector; equivalent writers may converge without a global lock and a corrupt selected candidate is refreshed by publishing/selecting another verified candidate rather than destructively replacing a candidate another reader may hold.

The shared-local store also implements private opportunistic structural hygiene owned by `mk`. Within fingerprint namespaces already touched by ordinary artifact activity, hygiene may quarantine/reclaim unselected immutable candidates and abandoned staging/selector residue while preserving the currently selected candidate. Readers remain unregistered because restoration is transactional: loss of a quarantined source candidate becomes a conservative execution miss before project destinations are committed. Publication and hygiene coordinate with crash-released POSIX FIFO activity tokens, so owner death becomes distinguishishable through kernel descriptor closure without PID inspection or timeout heuristics. Hygiene does not remove project-scoped freshness metadata and is not a retention policy: TTL/LRU, cache-size limits, whole-fingerprint eviction, global sweeping guarantees and a public cache-management command remain outside the baseline.
...
Remote/network artifact transport, cross-user artifact sharing/trust, whole-fingerprint cache eviction/size-based retention policy, parallel scheduling and remote execution remain unimplemented.
...
CURRENT-67   mk artifact restoration is execution-only and never makes plan inspection mutate declared project outputs
CURRENT-68   artifact restoration requires a complete verified candidate matching the current effective operation fingerprint; invalid cache state is a conservative miss
CURRENT-69   artifact restoration writes receiving-checkout freshness evidence and produces ordinary output/up-to-date evidence without synthesizing process results, so result observation still forces execution
...
CURRENT-71   artifact bytes are user-local and may be shared across canonical project roots by effective operation fingerprint while freshness metadata remains project-scoped
...
CURRENT-73   shared-local artifact publication exposes fully verified immutable candidates through an atomic per-fingerprint selector and does not require a global publication lock
CURRENT-74   equivalent concurrent publishers are idempotent and must not destructively replace another process's verified committed candidate
CURRENT-75   corrupt selected shared artifact state is a conservative miss refreshed through another immutable candidate; remote/cross-user artifact transport and whole-fingerprint retention/eviction policy remain outside the current baseline
CURRENT-76   mk privately performs opportunistic structural hygiene in shared-artifact fingerprint namespaces it touches, preserving the selected candidate while allowing unselected immutable candidates and abandoned staging/selector residue to be reclaimed
