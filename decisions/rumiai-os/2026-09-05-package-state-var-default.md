...
La grammatica esatta di `<state-instance>` resta aperta e dovra rispettare le regole RumiAI sui nomi filesystem controllati da RumiAI. Resta inoltre da risolvere esplicitamente l'eventuale ambiguita/collisione fra un nome package contenente `-` e una forma `<pkg>-<state-instance>`; non viene introdotto qui un escaping o namespace aggiuntivo.
...
## 14. Configurazione RumiAI non ancora gestita da `pkg`

Le root semantiche `conf`, `data`, `home`, `cache`, `log`, `run`, `tmp` sono strutture RumiAI generali e non richiedono una root intermedia `var/`.
...
Questa forma e coerente fisicamente con il caso standard `$m_ROOT/conf/<pkg>/`, ma `shell` e gli altri componenti/eseguibili RumiAI correnti non sono oggi necessariamente rappresentati da package gestiti tramite `pkg`.

Per la fase corrente questo e una **eccezione esplicita** al requisito di ownership tramite package: componenti RumiAI non ancora gestiti da `pkg` possono usare direttamente le root semantiche appropriate.

Non viene introdotto ora un virtual package per tali componenti.

La scelta fra:

```text
mantenere l'eccezione
introdurre in futuro package/virtual package RumiAI
adottare un'altra ownership esplicita
```

dovra essere rivalutata separatamente quando il modello `pkg` sara sufficientemente consolidato.
...
1. grammatica esatta di `<state-instance>` e collision policy con package name contenenti `-`;
...
14. futura ownership dei componenti RumiAI oggi esclusi da `pkg`, inclusa l'eventuale introduzione di virtual package.
...
PKG-RUMIAI-STATE-01 componenti RumiAI non ancora gestiti da pkg possono usare direttamente le root semantiche come eccezione esplicita corrente
PKG-RUMIAI-STATE-02 nessun virtual package RumiAI e fissato nella baseline corrente; la decisione verra rivalutata
PKG-STATE-12   questa decisione non reintroduce migration framework, resolver, generations, inventory o @package v0
```
