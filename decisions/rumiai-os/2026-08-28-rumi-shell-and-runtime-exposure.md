# Decisione — Rumi shell e esposizione del runtime
...
La shell Rumi eredita l'environment inizializzato da RumiAI e dispone del namespace dei comandi Rumi nel proprio `PATH`.

Il runtime viene esposto dentro l'ambiente Rumi tramite un singolo entrypoint interno:

```text
$RumiAI_BIN_DIR/rumiai-os -> ../rumiai-os
```

Poiché phase 1 prepende `RumiAI_BIN_DIR` al `PATH`, dentro la Rumi shell:
...
La shell Rumi deve poter avere un prompt chiaramente riconoscibile e personalizzabile.
...
Per Bash, una Rumi-specific rc file consente prompt, alias e altre preferenze senza caricare automaticamente i normali startup file host.

Per il fallback POSIX `sh`, la variabile `ENV` può puntare a una Rumi-specific rc file assoluta per la shell interattiva.

## Relazione con il command interpreter

Dentro la shell Rumi:
