# Handoff — macOS Rumi shell pass and Bash banner fix
...
## Physical macOS Rumi shell result
...
This physically validates on macOS:

- no-argument `rumiai-os` enters the Rumi shell;
- Bash is selected by default;
- Rumi-specific bashrc is loaded;
- recognizable `[RumiAI]` prompt is active;
- physical/canonical Rumi root state is preserved;
- Rumi `bin/` is available through PATH;
...
`lib/shell.lib` now sets and exports:
...
Physical macOS has also passed:
...
Full product execution on Linux is still pending.
...
The RumiAI prompt should appear without Apple's zsh migration/deprecation banner.

After confirming clean startup, continue with POSIX `sh` fallback and remaining phase-0 pathname/symlink edge cases before Linux full-product validation.
