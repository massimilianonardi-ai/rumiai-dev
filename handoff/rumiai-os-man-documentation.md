...
- `rumiai-os@fce90adde7ee5901a6c71560a7cf72b8df9492b2` materialized the 18 command topics that were previously missing. Together with the pre-existing topics, every command identity at that checkpoint had an owner-local manual topic:
  - technical/root and sys: `m`, `digest`, `extract`, `http-fetch`, `lang`, `lang-set`, `log`, `manual`, `menu-ext`, `menu-ext-adv`, `menu-ext-adv-fs`, `mk`, `pager`, `pkg`, `pkg-analyze`, `read-key`, `readc`, `shell`, `srv`, `state-path`;
...
- `ipc.lib.sh` is now covered by `res/sys/manual/ipc.lib.sh` after the dedicated IPC hardening work. Its aligned public API is exactly `ipc_create`, `ipc_open`, `ipc_write`, `ipc_read`, `ipc_close`, `ipc_sync`, `ipc_cancel` and `ipc_destroy`; `_ipc_*` helpers remain internal. Dedicated formal validation passed on hosted Linux/x86_64 and Darwin/arm64 with filesystem audit `CLEAN` for both runs.
- Later product work consolidated the public platform-selection surface under `osarch` with query, `show`, `update` and `set`, and added `res/sys/manual/osarch`. The obsolete compatibility wrappers have since been removed from the current command surface together with their manual topics.
