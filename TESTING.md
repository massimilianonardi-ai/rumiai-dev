...
The normal validation isolation granularity is `session`. For a homogeneous task selection, one disposable environment spans that selected session so cumulative state effects remain observable. Complete-product validation is heterogeneous: its baseline and each active requirement profile are separate session environments, preventing one group's prepared prerequisites from changing another group's starting conditions. `rumiai-validate` also provides a stronger `test` isolation mode in which each discovered test receives a newly created disposable environment. That mode mechanically prevents one test's filesystem state from becoming another test's starting state. Test correctness must not depend on which isolation granularity is selected.
